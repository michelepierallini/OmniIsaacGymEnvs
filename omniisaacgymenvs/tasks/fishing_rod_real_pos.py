...
class FishingRodTaskPos(RLTask):
    ...
    def get_fishingrod(self):
        ...
        self._name_joint = ['fishing_actuator_joint'] + [f'Joint_{i}' for i in range(1, 22)]
    ...
    def pre_physics_step(self, actions) -> None:
        ...
        for _ in range(self._decimation):  
            if self._env._world.is_playing():   
                # ## safety mechanism to fix the fishing rod tip. I sove it using directly the action_scale
                # if (self.dof_pos[:, 0] > self.torch_deg_sat).all():
