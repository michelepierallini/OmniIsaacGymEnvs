...
def solve_mpc_nlp(current_state, 
                  pos_d, 
                  vel_des, 
                  N_mpc, 
                  DT, 
    ...
    if casadi_int:
        dae = {'x': X0, 'p': U, 'ode': dyn_func(X0, U)}
        casadi_integrator = integrator('integrator', 'cvodes', dae, {'tf': h}) 
        F_dynamics = Function('F_dynamics', 
                            [X0, U], 
                            [casadi_integrator(x0=X0, p=U)['xf']], 
                            ['x0', 'u0'], 
                            ['xf'])
    else:
        X = X0
    ...
    if wanna_option:
        ...
        opts = {'ipopt.max_iter': ConstFishSiply.max_iter_ipopt, 
                'warn_initial_bounds': ConstFishSiply.warn_initial_bounds, 
                'ipopt.tol': ConstFishSiply.ipopt_tol,
                "ipopt.print_level": 0}
        ...
        solver = nlpsol('solver', 'ipopt', prob, {"ipopt.print_level": 3})
...
def run_mpc_control(pos_d=[2.58, 0.15], 
    ...
    ## This can be used for setting J
    ...
    for step in range(num_steps):
        ...
        result = evaluate_mx()
        if wanna_info:
            print("[INFO]: Type of result   : ", type(result))
            print("[INFO]: Result contents  : ", result)
...
if __name__ == "__main__":
    ...
    print(f"[INFO]: Control history len   : {len(control_history_np)}")
    print(f"[INFO]: Control history shape : {np.shape(control_history_np)}")
